...
describe('list.set', () => {
  ...
  describe('#nub', () => {
    ...
    it('handles an array', () => {
      expect(set.nub([1, 2, 2, 3, 3, 3])).toEqual([1, 2, 3])
    })

    it('handles a string', () => {
      expect(set.nub('abbccc')).toBe('abc')
    })
  })
  ...
  describe('#unionBy', () => {
    ...
    it('handles an empty array', () => {
      expect(set.unionBy(f)([1, 2, 3])([])).toEqual([1, 2, 3])
      expect(set.unionBy(f)([])([1, 2, 3])).toEqual([1, 2, 3])
    })

    it('handles an empty string', () => {
      expect(set.unionBy(f)('abc')('')).toBe('abc')
      expect(set.unionBy(f)('')('abc')).toBe('abc')
    })

    it('handles an array', () => {
      expect(set.unionBy(f)([1, 2, 3])([1, 2, 3])).toEqual([1, 2, 3])
      expect(set.unionBy(f)([1, 2, 3])([2, 3, 4])).toEqual([1, 2, 3, 4])
      expect(set.unionBy(f)([1, 2, 3])([4, 5, 6])).toEqual([1, 2, 3, 4, 5, 6])
      expect(set.unionBy(f)([1, 1])([1])).toEqual([1, 1])
    })

    it('handles a string', () => {
      expect(set.unionBy(f)('abc')('abc')).toBe('abc')
      expect(set.unionBy(f)('abc')('bcd')).toBe('abcd')
      expect(set.unionBy(f)('abc')('def')).toBe('abcdef')
      expect(set.unionBy(f)('aa')('a')).toBe('aa')
    })
  })
