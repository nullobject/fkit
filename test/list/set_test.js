...
describe('list.set', function() {
  ...
  describe('#subsequences', function() {
    it('should handle arrays', function() {
      expect(set.subsequences([])).to.be.eql([[]]);
      expect(set.subsequences([1, 2, 3])).to.be.eql([[], [1], [2], [1, 2], [3], [1, 3], [2, 3], [1, 2, 3]]);
    });

    it('should handle strings', function() {
      expect(set.subsequences('')).to.be.eql(['']);
      expect(set.subsequences('abc')).to.be.eql(['', 'a', 'b', 'ab', 'c', 'ac', 'bc', 'abc']);
    });
  });

  describe('#permutations', function() {
    it('should handle arrays', function() {
      expect(set.permutations([])).to.be.eql([[]]);
